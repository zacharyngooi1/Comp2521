...
void BSTreeLevelOrder (BSTree t)
{
	...
	while (!QueueIsEmpty(Q)) {
	    Item temp = QueueLeave(Q);
	    printf("%d ", temp->value);
        if (temp->left != NULL) {
            QueueJoin(Q,temp->left);
        }
        if (temp->right != NULL) {
            QueueJoin(Q,temp->right);
        }
	}
	dropQueue(Q);
...
int BSTreeNumLeaves (BSTree t)
{
	...
	int L_count = BSTreeNumLeaves(t->left);
	int R_count = BSTreeNumLeaves(t->right);
	int ans;
	
	if ((t->left == NULL) && (t->right == NULL)) {
	    // No need to include L_count and R_count.
	    ans = 1
    }
    else {
        ans = L_count + R_count;
